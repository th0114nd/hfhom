...
def vectorMaxPreimage(f, vs):
    '''Finds the vector v in vs with maximum f(v). Raises a ValueError if
    vs is an empty collection.'''
    return max([(f(v), v) for v in vs])[1]
...
def linear_combos(vlist, clist):
    for c in clist:
        yield np.dot(vlist, c)

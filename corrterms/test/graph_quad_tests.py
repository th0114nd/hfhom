...
# Graph tests 
#
def t_disabled_est_last_ancestor():
    num_nodes = 12
...
def t_disabled_est_edges_regions():
    '''Testing files not found...'''
...
def t_disabled_est_maximal_subtree():
    '''Testing files not found.'''
...
#
def t_disabled_est_symmetric():
    mat1 = np.matrix([[1,2,3],[0,4,5],[0,0,6]])
    mat2 = symmetric(mat1)
    assert np.all(mat2 == np.matrix([[1,2,3],[2,4,5],[3,5,6]]))

def t_disabled_est_circuit_and_shared_edges():
    #
    # test circuit
    #
...
def t_disabled_est_shared_edges_more():
...
def t_disabled_est_quad_form():
    # Test P1
    assert np.array_equal(quad_form(edge_list_p1, minus_maximal_subtree(\
        edge_list_p1, maximal_subtree(edge_list_p1, nodes_p1)), nodes_p1),\
        np.array([[-2,0],[0,-1]]))
    
    # Test P2
    # make the array for p2...
    p2_array = -1 * np.ones((7, 7), dtype=np.int)
    for i in range(3):
        p2_array[i,i] = -2
    for i in range(3, 7):
        for j in range(3, 7):
            if i != j:
                ...
                p2_array[i,i] = -3
    assert np.array_equal(quad_form(edge_list_p2, minus_maximal_subtree(\
        edge_list_p2, maximal_subtree(edge_list_p2, nodes_p2)), nodes_p2),\
        p2_array)
    
    # Test P3
    # make the array for p3...
    p3_array = -1 * np.ones((6, 6), dtype=np.int)
    p3_array[5] = [0 for i in range(6)]
    p3_array[:,5] = [0 for i in range(6)]
    for i in range(5):
        p3_array[i,i] = -2
    p3_array[5,5] = -1
    assert np.array_equal(quad_form(edge_list_p3, minus_maximal_subtree(\
        edge_list_p3, maximal_subtree(edge_list_p3, nodes_p3)), nodes_p3),\
        p3_array)
    
    # Test P4
    # make the array for p4...
    p4_array = np.zeros((10, 10), dtype=np.int)
    for i in range(3):
        for j in range(3, 7):
            p4_array[i,j] = -1
            p4_array[j,i] = -1
    ...
    for i in [3,4,5,6]:
        for j in [3,4,5,6]:
            if i != j:
                ...
                p4_array[i,i]=-3
    assert np.array_equal(quad_form(edge_list_p4, minus_maximal_subtree(\
        edge_list_p4, maximal_subtree(edge_list_p4, nodes_p4)), nodes_p4),\
        p4_array)
    
    # Test K1 10x-2-1
    assert np.array_equal(quad_form(edge_list_k1, minus_maximal_subtree(\
        edge_list_k1, maximal_subtree(edge_list_k1, nodes_k1)), nodes_k1),\
        np.array([[-5,-2,-1],[-2,-5,1],[-1,1,-4]]))
    
    # Test K2 7x-1-2
    assert np.array_equal(quad_form(edge_list_k2, minus_maximal_subtree(\
        edge_list_k2, maximal_subtree(edge_list_k2, nodes_k2)), nodes_k2),\
        np.array([[-2,-1,-1,-1],[-1,-2,-1,-1],[-1,-1,-2,-1],\
                     [-1,-1,-1,-4]]))
    
    # Test K3 23x-11-1
    edge_list_k3, nodes_k3 = knotilus_load_objects('23x-11-1')
    k3_array = np.zeros((12, 12), dtype=np.int)
    for i in range(9):
        k3_array[i][i] = -2
    k3_array[11, 11] = -2
    ...
    k3_array[9,11] = k3_array[10,11] = k3_array[11,9] = k3_array[11,10] =\
        -1
    assert np.array_equal(quad_form(edge_list_k3, minus_maximal_subtree(\
